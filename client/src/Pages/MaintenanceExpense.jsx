...
const RecordMaintenance = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/add-maintenance-expenses', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setSuccessMessage('Maintenance expense recorded successfully');
        setTimeout(() => {
    ...
    <Container>
      <Row>
        <Col sm="6" className="mx-auto mt-5">
          <h2 className="mb-4 text-center">Record Maintenance Expenditure</h2>
          {successMessage && (
